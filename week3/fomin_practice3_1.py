...
def policy_evaluation(env: FrozenLakeEnv, policy: Policy, gamma: float, iter_n: int) -> QTable:
...
def train(env: FrozenLakeEnv, epochs: int, policy_evaluation_steps: int, gamma: float) -> Policy:
    ...
    for k in range(epochs):
        q_values = policy_evaluation(env, policy, gamma, policy_evaluation_steps)
...
def main():
    ...
    pd.DataFrame(output).to_csv("task1_1.csv", index=False)
    output = []
    for gamma in tqdm(range(9900, 10001, 1)):
        gamma /= 10000
        policy = train(env, epochs, policy_evaluation_steps, gamma)
        mean_reward = evaluate(env, policy)
        output.append({
    ...
    pd.DataFrame(output).to_csv("task1_2.csv", index=False)
