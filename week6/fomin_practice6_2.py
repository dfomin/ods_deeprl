...
class PPO(nn.Module):
    ...
    def get_action(self, state):
        ...
        dist = Normal(output[:2], torch.exp(output[2:]))
        action = dist.sample()
        return action.numpy().reshape(2)
    ...
    def fit(self, states, actions, rewards, dones):
        ...
        output = self.pi_model(states).T
        mean, log_std = output[:2].T, output[2:].T
        ...
        for epoch in range(self.epoch_n):
            ...
            for i in range(0, returns.shape[0], self.batch_size):
                ...
                b_output = self.pi_model(b_states).T
                b_mean, b_log_std = b_output[:2].T, b_output[2:].T
...
def train(env: gym.Env, agent: PPO) -> list[float]:
    ...
    for episode in range(episode_n):
        ...
        # evaluate(gym.make('LunarLander-v2', continuous=True, render_mode="human"), agent, 1, t_max)
