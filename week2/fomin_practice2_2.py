...
class Network(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, hidden_dims: list[int], lr=1e-3):
        super().__init__()
...
class DeepCEMAgent:
    def __init__(self, state_dim: int, quantile: float, epsilon_min: float = 1e-2, epsilon_decay: float = 0.99):
        self.model = Network(state_dim, 1, [64, 64])
        self.epsilon = 1
    ...
    def get_action(self, state: np.ndarray, eval: bool = False) -> int:
        ...
        if eval:
            return pred
        probs = pred + np.random.normal(0, self.epsilon)
        probs = np.clip(probs, -2, 2)
        return probs
...
def main():
    ...
    for quantile in tqdm(elite_quantiles):
        for count in tqdm(trajectories_counts, leave=False):
            agent = DeepCEMAgent(env.observation_space.shape[0], quantile)
            result = train(env, agent, count, 200, 100)
            for item in result:
                output.append({
                    "epoch": item[0],
                    "reward": item[1],
